...
Scene 3 (4.6–7.0s): on "Two thin Shopware backends", two connectors draw down in parallel (`svg-path-draw`) into two side-by-side tile cards: `storefront/api` (mono: `ucp_client.py · shopware_backend.py`) and `merchant/api` (mono: `admin_transport.py · staging.py`), both under the mono label `github.com/agentic-commerce-lab/shopware-claude-commerce`. Reveal left then right via **staggered reveal** (`dynamic-content-sequencing`).
