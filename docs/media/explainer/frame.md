...
shadows: { card: "0 1px 3px #000@30%, 0 8px 32px #000@25%", none: "none" }
...
- **Inverted register (brief: "modern, dark, technical").** The preset's "cream paper" is the deep navy ground `#0B1220`; "ink" is the light text `#F3F6FA`; "tile" / "tile-strong" are the half-step lighter surfaces `#111A2E` / `#1B2740`; the code / terminal surface (`navy`, `navy-soft`, `navy-elev`) is one step *darker* than the ground and lifted by the hairline. "Coral" is Shopware blue `#189EFF` — the ONE voltage per frame. `warm` `#D97757` (Claude / Anthropic) is a second, rationed accent: only for the Claude / blueprint / model side of a diagram or a single word, never a fill, never twice in one frame beside the blue. `muted` `#8A94A6` is the secondary text tone. Pure white and pure black stay banned; the ground is always the navy.
...
## Fonts — local `@font-face` (staged in `assets/fonts/`, OFL-licensed)
...
Frames live in `compositions/frames/`, so from a frame file the path is `../../assets/fonts/...`; the assembled `index.html` at the project root uses `assets/fonts/...`. Weights available: 400 and 700 only (600 / 800 in the brand token list are clamped to 700).
