...
├── cases/
│   ├── shopping/*.yaml    62 cases, grouped by area
│   └── merchant/*.yaml    39 cases
└── tests/
...
Options: `--suite shopping|merchant|all`, `--set ci|full`, `--mode replay|live`,
...
| `regex` | pattern · `{pattern, must_match, flags}` | `re.search` outcome matches `must_match` (default flags `is`) |
...
- Injected `seen_products` / `seen_listings` are **provenance**, not conversation: the
  ...
  product is already "in view", give the case a `history` line that puts it there (as
  `shop-provenance-002` does), or name the product in the message.
- With `close_on_presentation` on, a turn may end on a card plus chips with one line of
  ...
  payload (`ui_payload_ids`, `ui_payload_has`, `staged_change_kind`) where the card is
  the answer, and keep `text_contains` for wording that must appear in prose.
